 ...
 */
namespace two {
  type FirstLetter<word> = word extends `${infer firstLetter}${string}` ? firstLetter : word;
...
namespace bonus {
   ...
   */
  namespace four {
    type EndOfWord<word> = word extends `${infer first}${infer rest}` ? `${rest}` : never;
   ...
   */
  namespace five {
    type GetFirstFolder<path> = path extends `/${infer first}/${string}` ? `${first}` :
    path extends `/${infer first}` ? `${first}` :
    path extends `${infer first}/${string}` ? `${first}` : never;
