 ...
 */
namespace one {
  type Head<tuple extends unknown[]> = tuple extends [infer first, ...unknown[]]
    ? first
    : never;
 ...
 */
namespace two {
  type DropFirst<tuple extends unknown[]> = tuple extends [
    unknown,
    ...(infer rest)
  ]
    ? rest
 ...
 */
namespace four {
  type Concat<tuple1 extends unknown[], tuple2 extends unknown[]> = [
    ...tuple1,
    ...tuple2
  ];
 ...
 */
namespace five {
  type Last<tuple extends unknown[]> = tuple extends [...unknown[], infer last]
    ? last
    : never;
