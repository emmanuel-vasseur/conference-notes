 ...
 */
namespace one {
  type If<condition extends boolean, branch1, branch2> = condition extends true
    ? string
    : number;
 ...
 */
namespace two {
  type GetTypeOfName<input> = input extends { name: infer s } ? s : never;
 ...
 */
namespace three {
  type UnwrapPromise<input> = input extends Promise<infer type> ? type : input;
